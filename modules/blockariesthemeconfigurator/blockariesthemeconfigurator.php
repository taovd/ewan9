...
class BlockAriesThemeConfigurator extends Module
{
	...
	{
		$this->name = 'blockariesthemeconfigurator';
		...
		parent::__construct();
		$this->displayName = $this->l('Aries Theme configurator');
		$this->description = $this->l('Configure the main elements of your Aries theme.');
	...
	public function createAjaxController()
	{
		...
		if (is_array($languages))
			foreach ($languages as $language)
				$tab->name[$language['id_lang']] = 'blockariesthemeconfigurator';
		$tab->class_name = 'AdminBlockAriesThemeConfigurator';
	...
	{
		if ($tab_id = (int)Tab::getIdFromClassName('AdminBlockAriesThemeConfigurator'))
		{
			$tab = new Tab($tab_id);
			$tab->delete();
		}
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			!$this->registerHook('AriesAdvertiseHome01') ||
			!$this->registerHook('AriesAdvertiseHome02') ||
			...
			Configuration::updateValue('BOXED_LAYOUT', 1) ||
	...
	{
		return (
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'blockariesthemeconfigurator`') &&
			Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'blockariesthemeconfigurator` (
	...
	protected function installFixture($hook, $id_image, $id_shop, $id_lang)
	{
		...
		$result &= Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'blockariesthemeconfigurator` (
	...
	public function installFixtures($languages = null)
	{
		...
		foreach ($languages as $language)
		{
			...
			for ($i = 1; $i < 4; $i++)
				$result &= $this->installFixture('ariesadvertisehome01', $i, $this->context->shop->id, $language['id_lang']);
			for ($i = 4; $i < 6; $i++)
				$result &= $this->installFixture('ariesadvertisehome02', $i, $this->context->shop->id, $language['id_lang']);
			for ($i = 6; $i < 7; $i++)
	...
	{
		$images = array();
		if (count(Db::getInstance()->executeS('SHOW TABLES LIKE \''._DB_PREFIX_.'blockariesthemeconfigurator\'')))
			$images = Db::getInstance()->executeS('SELECT image FROM `'._DB_PREFIX_.'blockariesthemeconfigurator`');
		foreach ($images as $image)
			$this->deleteImage($image['image']);

		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'blockariesthemeconfigurator`') || !$this->_removeAjaxContoller() || !parent::uninstall())
			return false;
	...
	public function hookdisplayHeader()
	{
		...
		if ((Configuration::get('BOXED_LAYOUT')) == 1)
			$this->context->controller->addCss($this->_path.'views/css/full_width.css', 'all');
		else
			$this->context->controller->addCss($this->_path.'views/css/boxed.css', 'all');
	...
	public function hookAriesAdvertiseHome01()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('ariesadvertisehome01'),
			'hook' => 'ariesadvertisehome01'
		));

		return $this->display(__FILE__, 'hook.tpl');
	}
	public function hookAriesAdvertiseHome02()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('ariesadvertisehome02'),
			'hook' => 'ariesadvertisehome02'
		));
	...
	protected function getItemsFromHook($hook)
	{
		...
		SELECT *
		FROM `'._DB_PREFIX_.'blockariesthemeconfigurator`
	...
	protected function removeItem()
	{
		...
		if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockariesthemeconfigurator` WHERE id_item = '.(int)$id_item))
			$this->deleteImage($image);

		Db::getInstance()->delete(_DB_PREFIX_.'blockariesthemeconfigurator', 'id_item = '.(int)$id_item);

		if (Db::getInstance()->Affected_Rows() == 1)
		{
			Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'blockariesthemeconfigurator`
	...
	protected function updateItem()
	{
		...
		{
			if ($old_image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockariesthemeconfigurator` WHERE id_item = '.(int)$id_item))
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'blockariesthemeconfigurator` SET
					title = \''.pSQL($title).'\',
		...
		{
			if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockariesthemeconfigurator` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->deleteImage($image);
	...
	public function getContent()
	{
		...
		{
			Configuration::updateValue('BOXED_LAYOUT', (int)Tools::getValue('boxed'));
		...
		$html .= $this->renderBlockAriesThemeConfiguratorForm();
	...
	protected function renderBlockAriesThemeConfiguratorForm()
	{
		$id_shop = (int)$this->context->shop->id;
		$items = array();
		$hooks = array();
		...
		foreach ($this->languages as $language)
		{
			$hooks[$language['id_lang']] = array(
				'ariesadvertisehome01',
				'ariesadvertisehome02',
				'home',
				'top',
				'left',
				'right',
			...
			foreach ($hooks[$language['id_lang']] as $hook)
				$items[$language['id_lang']][$hook] = Db::getInstance()->ExecuteS('
					SELECT * FROM `'._DB_PREFIX_.'blockariesthemeconfigurator`
		...
		$this->context->smarty->assign('htmlitems', array(
			'items' => $items,
			'theme_url' => $this->context->link->getAdminLink('AdminBlockAriesThemeConfigurator'),
			'lang' => array(
	...
	protected function getConfigurableModules()
	{
		...
		if ($this->context->shop->getBaseURL())
		{
			...
			$desc = '<a class="btn btn-default" href="'.$url.'" onclick="return !window.open($(this).attr(\'href\'));" id="live_conf_button">'
				...
				.$this->l('This tool allows you to choose predifned color changes in your theme.');
		}
		else
		...
		return array(
			...
			array(
				...
				'name' => 'blockariesnewsletter',
				'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('blockariesnewsletter')) && $module->isEnabledForShopContext(),
				'is_module' => true,
			),
			array(
			...
			array(
				...
				'hint' => $this->l('This tool allows you to choose predifned color changes in your theme.'),
				'desc' => $desc
			),
			array(
			...
			array(
				'label' => $this->l('Display Fullmode'),
				'name' => 'boxed',
				'value' => (int)Tools::getValue('BOXED_LAYOUT', Configuration::get('BOXED_LAYOUT')),
			)
		);
	}
