			...
			<div id="image-block" class="clearfix">
				...
				{if $have_image}
					<span id="view_full_size">
						...
						{else}
							<img id="bigpic" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default')|escape:'html':'UTF-8'}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" width="{$thickboxSize.width}" height="{$thickboxSize.height}"/>
							{if !$content_only}
				...
				{else}
					<span id="view_full_size">
						<img itemprop="image" src="{$img_prod_dir}{$lang_iso}-default-thickbox_default.jpg" id="bigpic" alt="" title="{$product->name|escape:'html':'UTF-8'}" width="{$thickboxSize.width}" height="{$thickboxSize.height}"/>
						{if !$content_only}
						...
						{if isset($images)}
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}',thickboximage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
						...
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print rx-cart">
								<button type="submit" name="Submit" class="exclusive">
									<span><i class="icon-shopping-cart"></i>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='Add to cart'}{/if}</span>
								</button>
...
<div class="cd-tabs">
	<nav>
        <ul class="cd-tabs-navigation">
            ...
            {if $product->description}
            <li><a data-content="More" class="selected" href="#0">{l s='More info'}</a></li>
            {/if}
            {if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
        	<li><a data-content="Volume" href="#0">{l s='Volume discounts'}</a></li>
            {/if}
            {if isset($features) && $features}
            <li><a data-content="Data" href="#0">{l s='Data sheet'}</a></li>
            {/if}
            {if isset($packItems) && $packItems|@count > 0}
            <li><a data-content="Pack" href="#0">{l s='Pack content'}</a></li>
            {/if}
            <li><a data-content="#idTab5" href="#0">{$HOOK_PRODUCT_TAB}</a></li>
            {if isset($accessories) && $accessories}
            <li><a data-content="Accessories" href="#0">{l s='Accessories'}</a></li>
            {/if}
            {if (isset($product) && $product->description) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
               	
                {if isset($attachments) && $attachments}
                <li><a data-content="Download" href="#0">{l s='Download'}</a></li>
                	
                {/if}
                 {if isset($product) && $product->customizable}
                 <li><a data-content="product_customization" href="#0">{l s='Product customization'}</a></li>
                 {/if}
         {/if}
        ...
        {if isset($product) && $product->description}
            ...
            {if (isset($product) && $product->description) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
              ...
              {/if}
            {if isset($product) && $product->customizable}
            <li data-content="product_customization">
